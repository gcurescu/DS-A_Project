...
class Truck(Hubs):
    ...
    def deliver_route(self):
        while (len(self.regular_current_packages) or len(self.priority_current_packages)) != 0:  # Check for packages
            ...
            hr.append(int(self.hours));
            mint.append(int(self.minutes));
            sec.append(int(self.seconds));
...
pac = master_packages.package_list;
...
if selection == 1:
    ...
    t = input('Enter time (HH:MM 24h format): ');
    report_hour = int(t[0:2])
    if report_hour >= 0 or report_hour < 24:
        report_minute = int(t[3:5])
...
if gate1 == 1:
    print('ID    '+'Address    '+'Deadline    '+'City    '+'Zip Code    '+'    Weight'+'     State'+'     Time Delivered\n');
    for i in pac:
        print(i.package_id+'    '+i.address+'   '+i.deadline+'  '+i.city+'  '+i.zip_code+'  '+i.mass+'  '+i.state+' '+str(hr[count])+':'+str(mint[count])+':'+str(sec[count]))
        if(len(hr) > count+1 ):
            count+=1;
...
if gate2 == 1:
    print('ID    '+'Address    '+'Deadline    '+'City    '+'Zip Code    '+'    Weight'+'     State'+'     Time Delivered\n');
    for i in pac:
        if report_hour >= hr[count]:
            if (report_hour == hr[count] and report_minute >= mint[count]) or (report_hour > hr[count]):
                print(i.package_id+'    '+i.address+'   '+i.deadline+'  '+i.city+'  '+i.zip_code+'  '+i.mass+'  '+i.state+' '+str(hr[count])+':'+str(mint[count])+':'+str(sec[count]))
                if(len(hr) > count+1 ):
                    count+=1;
...
if gate3==1:
    print('Total Distance Convered: ' + str(truck_a.mileage + truck_b.mileage + truck_c.mileage))
